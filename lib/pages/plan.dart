...
//  will return a widget used as an indicator for the drop position
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Exportiert:', style: TextStyle(fontWeight: FontWeight.bold)),
                SizedBox(height: 8),
                Row(
                  children: [
                ...
                SizedBox(height: 16),
                Text('Die Datei kann in jeder beliebigen App gespeichert werden.',
            ...
            actions: [
              ...
              ElevatedButton.icon(
                onPressed: () {
                  ...
                  _shareExportFile(file, totalCategories, totalDishes, totalDays);
                },
  ...
  void _shareExportFile(File file, int categories, int dishes, int days) async {
    try {
      ...
      await Share.shareXFiles(
        [XFile(file.path)],
        text: 'Essensplan Export vom $dateStr\n\n'
            'Inhalt:\n'
  ...
  void _importData(BuildContext context) async {
    try {
      ...
      if (!importData.containsKey('appName') || importData['appName'] != 'Essensplan') {
        throw Exception('Ungültige Export-Datei. Nur Essensplan-Exports werden unterstützt.');
      }
      ...
      // Import-Modus wählen
      ImportMode? importMode = await _showImportModeDialog(context, 
          categoriesToImport, dishesToImport, daysToImport);
      
      if (importMode == null) return;

      // Import durchführen
      ImportResult importResult = await _performImport(
        importData, dayBox, dishBox, categoryBox, importMode);
  ...
  Future<ImportMode?> _showImportModeDialog(BuildContext context, 
      int categories, int dishes, int days) async {
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Import-Inhalt:', style: TextStyle(fontWeight: FontWeight.bold)),
              ...
              child: Wrap(
                ...
                alignment: WrapAlignment.center,
                  ...
                  _buildImportActionButton(
                    label: 'Nur neue\nhinzufügen',
                    icon: Icons.add,
                    onPressed: () => Navigator.of(context).pop(ImportMode.addOnly),
                  ),
                  _buildImportActionButton(
                    label: 'Smart Merge\n(empfohlen)',
                    icon: Icons.merge,
                    isPrimary: true,
                    onPressed: () => Navigator.of(context).pop(ImportMode.smartMerge),
                  ),
                  _buildImportActionButton(
                    label: 'Alles\nersetzen',
                    icon: Icons.refresh,
                    ...
                    onPressed: () => Navigator.of(context).pop(ImportMode.replaceAll),
                  ),
                  TextButton(
                    onPressed: () => Navigator.of(context).pop(),
  ...
  Future<ImportResult> _performImport(
    ...
    if (importData.containsKey('categories')) {
      ...
      for (var categoryData in categories) {
        ...
        Category? existingCategory = _findCategoryByName(categoryBox, categoryName);
        ...
        if (existingCategory == null) {
          ...
          if (mode == ImportMode.smartMerge) {
            bool updated = await _mergeCategoryData(existingCategory, categoryData);
            if (updated) {
    ...
    if (importData.containsKey('dishes')) {
      ...
      for (var dishData in dishes) {
        ...
        if (existingDish == null) {
          ...
          Dish dish = await _createDishFromData(dishData, importedCategories, categoryBox);
          ...
          if (mode == ImportMode.smartMerge) {
            bool updated = await _mergeDishData(existingDish, dishData, importedCategories, categoryBox);
            if (updated) {
    ...
    if (importData.containsKey('days')) {
      ...
      for (var dayKey in days.keys) {
        ...
        if (existingDay == null || existingDay.entries == null || existingDay.entries!.isEmpty) {
          // Neuen Tag erstellen
          ...
          bool updated = await _mergeDayData(existingDay, dayData, importedDishes, dishBox);
          if (updated) {
  ...
  Category? _findCategoryByName(Box<Category> categoryBox, String name) {
    return categoryBox.values
        .cast<Category?>()
        .firstWhere((cat) => cat?.name.toLowerCase() == name.toLowerCase(), orElse: () => null);
  }

  Dish? _findDishByName(Box<Dish> dishBox, String name) {
    return dishBox.values
        .cast<Dish?>()
        .firstWhere((dish) => dish?.name?.toLowerCase() == name.toLowerCase(), orElse: () => null);
  }

  Future<bool> _mergeCategoryData(Category existing, Map<String, dynamic> newData) async {
    bool updated = false;
  ...
  Future<bool> _mergeDishData(
    ...
    if (newData.containsKey('categories') && newData['categories'] != null) {
      ...
      Set<String> existingCatNames = existing.categories?.map((c) => c.name.toLowerCase()).toSet() ?? {};
      ...
      for (var catData in newData['categories']) {
        ...
        if (catName.isNotEmpty && !existingCatNames.contains(catName.toLowerCase())) {
          Category? category = importedCategories[catName] ?? _findCategoryByName(categoryBox, catName);
          if (category != null) {
  ...
  Future<Day> _createDayFromData(
    ...
    if (dayData.containsKey('entries')) {
      for (var dishData in dayData['entries']) {
        String dishName = dishData['name'] ?? '';
        Dish? dish = importedDishes[dishName] ?? _findDishByName(dishBox, dishName);
  ...
  Future<bool> _mergeDayData(
    ...
    Set<String> existingDishNames = existing.entries?.map((d) => d.name?.toLowerCase() ?? '').toSet() ?? {};
    List<Dish> allDishes = List.from(existing.entries ?? []);
    bool updated = false;
    
    for (var dishData in dayData['entries']) {
      String dishName = dishData['name'] ?? '';
      if (dishName.isNotEmpty && !existingDishNames.contains(dishName.toLowerCase())) {
        Dish? dish = importedDishes[dishName] ?? _findDishByName(dishBox, dishName);
        if (dish != null) {
          allDishes.add(dish);
          updated = true;
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                if (result.importedCategories > 0 || result.importedDishes > 0 || result.importedDays > 0) ...[
                  Text('Neu hinzugefügt:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green)),
                  if (result.importedCategories > 0) _buildStatRow(Icons.category, '${result.importedCategories} Kategorien'),
                  if (result.importedDishes > 0) _buildStatRow(Icons.restaurant, '${result.importedDishes} Gerichte'),
                  if (result.importedDays > 0) _buildStatRow(Icons.calendar_today, '${result.importedDays} Tage'),
                  SizedBox(height: 8),
                ],
                if (result.mergedCategories > 0 || result.mergedDishes > 0 || result.mergedDays > 0) ...[
                  Text('Zusammengeführt:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)),
                  if (result.mergedCategories > 0) _buildStatRow(Icons.merge, '${result.mergedCategories} Kategorien'),
                  if (result.mergedDishes > 0) _buildStatRow(Icons.merge, '${result.mergedDishes} Gerichte'),
                  if (result.mergedDays > 0) _buildStatRow(Icons.merge, '${result.mergedDays} Tage'),
                  SizedBox(height: 8),
                ],
